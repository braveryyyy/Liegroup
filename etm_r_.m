...
%             0,           0,  0 ];
	...
	%%     fi     dvn    dpos    eb       db
	Ft = [ -(Mav*askew(ins.vn)+askew(ins.eth.wnin))    Mav    -(Mp1+Mp2)    -ins.Cnb  O33 
           askew(ins.vn)*askew(ins.eth.wnie)+askew(ins.eth.gn)    -askew(2*ins.eth.wnie+ins.eth.wnen)    askew(ins.vn)*Mp1     -askew(ins.vn)*ins.Cnb      -ins.Cnb 
           Mpv*askew(ins.vn)     -Mpv Mpp      O33   O33
           zeros(6,15)  ];
    Gammak=[-ins.Cnb  zeros(3,3)   zeros(3,13)
             -askew(ins.vn)*ins.Cnb   -ins.Cnb   zeros(3,13)
             zeros(3,19)
             zeros(10,19)];
    Fk = Ft*ins.nts;
