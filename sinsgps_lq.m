function [avp, xkpk, zkrk, sk, ins, kf,ins_new,count] = sinsgps_lq(imu, gps, ins, davp, imuerr, lever, dT, rk, Pmin, Rmin, fbstr, isfig)
...
global glv
    ...
    for k=1:nn:len-nn+1
        ...
        if strcmp(kf.lqmode,'right')==1
            [kf.Phikk_1(1:15,1:15),kf.Gammak]=etm_r_(ins);
        elseif strcmp(kf.lqmode,'tra')==1
            kf.Phikk_1 = kffk(ins);
            kf.Gammak=Gammak;
        else
            [kf.Phikk_1(1:15,1:15),kf.Gammak]=etm_l_(ins);
        end
        kf = kfupdate(kf);
...
%                 end
            ...
            if strcmp(kf.lqmode,'right')==1
               kf.Hk=[askew(ins.vn),-eye(3),zeros(3,9),ins.CW,ins.anbar;  %% 右误差
               zeros(3,3),zeros(3,3),eye(3),zeros(3,6),-ins.MpvCnb,-ins.Mpvvn];
            elseif strcmp(kf.lqmode,'left')==1
                  kf.Hk=[zeros(3,3),-ins.Cnb,zeros(3,9),-ins.Cnb*(-ins.CW),-ins.Cnb*(-ins.anbar);  %% 左误差
                zeros(3,6),eye(3),zeros(3,6),-ins.MpvCnb,-ins.Mpvvn]; 
            end
            kf = kfupdate(kf, zk, 'M');
            zkrk(kiz,:) = [zk; diag(kf.Rk); t];  kiz = kiz+1;
            if strcmp(kf.lqmode,'tra')==1
                 [kf, ins] = kffeedback(kf, ins, nts);
            else
            [kf, ins] = kffeedback_lq(kf, ins, nts);
            end
            flag=1;
        else
            flag=0;
