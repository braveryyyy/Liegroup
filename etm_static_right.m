function [Fk,Gammak] = etm_static_right(ins)
...
%             0,           0,  0 ];
	...
	%%     fi     dvn    dpos    eb       db
	Ft = [ -(askew(ins.eth.wnie))    O33        -ins.Cnb  O33 
           askew(ins.eth.gn)    O33       O33      -ins.Cnb 
           zeros(6,12)  ];
    Gammak=[-ins.Cnb  zeros(3,3)   zeros(3,6)
             O33   -ins.Cnb   zeros(3,6)
             zeros(6,12)];
    Fk = Ft*ins.nts;
