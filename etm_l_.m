...
%             0,           0,  0 ];
	...
	%%     fi     dvn    dpos    eb       db
     nts=ins.nts;
	Ft = [-askew(ins.wib)   -ins.Cnb'*Mav*ins.Cnb    ins.Cnb'*(Mp1+Mp2)    -eye(3)  O33 
           -askew(ins.fb)    [ins.Cnb'*askew(ins.vn)*Mav*ins.Cnb-askew(ins.wib)-askew(ins.Cnb'*ins.eth.wnie)]    -ins.Cnb'*askew(ins.vn)*(2*Mp1+Mp2)     zeros(3,3)      -eye(3) 
           zeros(3,3)    -Mpv*ins.Cnb    Mpp     O33      O33
           zeros(6,15)] ;
    Gammak=[-eye(3)  zeros(3,3)   zeros(3,13)
             zeros(3,3)   -eye(3)   zeros(3,13)
             zeros(13,19)];
     Fk = Ft*nts;
