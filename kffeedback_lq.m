function [kf, ins, xfb] = kffeedback(kf, ins, T_fb,eth, fbstr)
    ...
    for k=1:length(fbstr)
        switch fbstr(k)
            case 'a',
                ...
                Q=askew(tem0);
                delat_R=eye(3)+sin(norm(tem0))/norm(tem0)*Q+(1-cos(norm(tem0)))/(norm(tem0))^2*Q^2;
                ins.Cnb=delat_R*ins.Cnb;
                end
                ins.qnb=m2qua(ins.Cnb);
            case 'v', 
                  idx = 4:6;  
                  if strcmp(kf.lqmode,'left')==1
                      X=ins.Cnb*xfb_tmp(idx);
                      ins.vn=ins.vn+tem_Cnb*xfb_tmp(idx);
                  elseif strcmp(kf.lqmode,'tra')==1
                      ins.vn = ins.vn - xfb_tmp(idx);
                  else
                  ins.vn=ins.vn-cross(ins.vn,xfb_tmp(1:3))+xfb_tmp(idx);
                  end
            case 'p',
                idx = 7:9  
                  if strcmp(kf.lqmode,'left')==1
%                        ins.pos=ins.pos+tem_Cnb*xfb_tmp(idx);
                       ins.pos = ins.pos - xfb_tmp(idx);
                  elseif strcmp(kf.lqmode,'tra')==1
                      ins.pos = ins.pos - xfb_tmp(idx);
                  else
%                       X=-cross(ins.pos,xfb_tmp(1:3))+xfb_tmp(idx);
%                   ins.pos=ins.pos+X;
                       ins.pos = ins.pos - xfb_tmp(idx);
                  end
            case 'e',
